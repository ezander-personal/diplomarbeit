...
\eqnl[svdkomp]{\vec c_i = \sum_{k=1}^N \frac{1}{\sigma_i\sqrt N} s_{ki} \x_k.} 
Wegen $d'\ll N$ ist die Darstellung allerdings nicht eindeutig. Die $s_{ki}$ bilden 
eine $\minembed\times N$ Matrix
\eqn{\mat S = 
  \left( \begin{array}{ccc}
    s_{11} & \dots & s_{1\minembed} \\
    \vdots & \ddots & \vdots \\
    s_{N1} & \dots & s_{N\minembed} \\
  \end{array} \right),
}
...
$\vec s_i$\footnote{Sonst h"atte die Matrix $\mat S$ den
  Rang $\rang(\mat S)<\minembed$, und die $\vec c_i$ k"onnten nach \eqnref{svdkomp} nicht
...
Transponierten, so erhalten wir} Daraus folgt unter Ausnutzung der vorausgesetzten Orthonormalit"at der $\vec c_i$:
...
\comment{Dies ist i.allg.\  nicht die einzige L"osung 
...
untersuchen. 
Als Beispiel m"oge hier wieder eine Zeitreihe des Lorenz-Attraktor dienen. 
