...
\eqnl[crosskorr]{C(X,Y)=\lim_{T\to\infty}\frac{1}{2T}\int_{-T}^{+T} (x(t)-\bar
  x)(y(t)-\bar y) \mathrm{dt} } Hierbei sind $x(t)$ und $y(t)$ die Werte der Signale zur
...
\eqnl[autokorr2]{\ac(k)=\frac{1}{N-k}\sum_{i_1}^{N-k} (x_i-\bar x)(x_{i+k}-\bar x)} mit
\eqnl[acmean]{\bar x = \frac{1}{N}\sum_{i=1}^N x_i} Die aufeinanderfolgenden Koordinaten
...
\begriff(Information)\footnote{In der Informationstheorie wird statt des nat"urlichen
  ...
  e\,\bit\,$miteinander verkn"upft.}  \eqn{H(X)=-\sum_i \Prob_X(i) \ln \Prob_X(i)} Die
...
mittlere Information der kombinierten Messung \eqn{H(X,Y)=-\sum_{i,j} \Prob_{XY}(i,j) \ln
  \Prob_{XY}(i,j)} wobei $\Prob_{XY}(i,j)$ die Verbundwahrscheinlichkeit ist, da"s bei der
...
n"amlich \eqn{H(Y|X)=H(X,Y)-H(X)} Hierbei bezeichnet $H(Y|X)$ die mittlere Information der
...
\eqnl[genredundancy]{R(X,Y)=H(X)+H(Y)-H(X,Y)} Die Redundanz $R$ wird genau dann minimal,
...
\eqnal[redundancy2]{R(X,\delay)&=&2H(X)-H(X,X_\delay) \nonumber \\
  ...
  \Prob_{x,\delay}(i,j) } wobei $\Prob_{x,\delay}(i,j)$ die Wahrscheinlichkeit ist, da"s
