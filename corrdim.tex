...
\eqnl[gendim]{D_q = \frac{1}{q-1} \lim_{\eps\to 0} \frac{\log\left(\sum_i \Prob_i^q\right)}{\log(\eps)}}
F"ur $q=2$ ergibt sich die sogenannte Korrelationsdimension $\corrdim$.
\eqn{\corrdim = \lim_{\eps\to 0} \frac{\log\left(\sum_i \Prob_i^2\right)}{\log(\eps)}}
...
Punktepaare
\eqnl[cintpredef]{\sum_i \Prob_i^2 = \frac{1}{N(N-1)} \, \# \left\{ (i,j) \, \vert \, i \neq j \land  \norm{\x_i-\x_j}\leq \eps \right\} } 
Die rechte Seite von \eqnref{cintpredef} ist das sogenannte
...
\eqnl[cdimdef]{\corrdim = \lim_{\eps\to 0} \frac{\log(C(\eps))}{\log(\eps)}}
Aufgrund der Unabh"angigkeit von $D_q$ von der speziellen Form der Partitionierung in
\eqnref{gendim}, ist das Korrelationsintegral (im Grenzfall $\eps\to0$) unabh"angig von
...
{Korrelationsintegral f"ur eine Zeitreihe ($N=7\times 10^5$)des R"osslerattraktor mit $\rho=2^{1/10}$,
...
\eqnref{gendim} erwartete Skalierungsverhalten $C(r)\propto r^\nu$, wobei die Konstante
...
{Ableitung des Logarithmus des Korrelationsintegrals auf \psref{corrintperf}. In die
...
Berechnung verwendet wird. \psref{corrslpperf} liefert trotzdem wichtige Informationen zur 
...
Die Steigung des Korrelationsintegrals im Bereich $[\rmin,\rmax[$ kann nat"urlich auch
durch Bestimmung einer Regressionsgeraden berechnet werden. Die Steigung dieser Geraden
ist
\eqn{\mcorrdim = \frac{\sum\limits_i (\log C(r_i)-\overline{\log C(r)}(\log r_i
-\overline{\log r_i}))}{\sum\limits_i(\log r_i-\overline{\log r})^2}}
...
Bereich (mit Steigung Null) besitzt. F"ur das Korrelationsintegral aus \psref{corrintperf} 
...
\eqn{\corrdim=-\frac{m}{\sum_{i=1}^m \log r_i}=-\frac{1}{<\log r_i>} }
...
\eqn{\corrdim(\rmax) = \int\limits_0^\rmax \abl{C(r)}{r}\frac{\log r}{C(\rmax)}\d r }
Da wir das Korrelationsintegral jedoch nur f"ur diskrete Werte $r_i$ bestimmen k"onnen,
geht die obige Gleichung "uber in
\eqn{\corrdim(r_n) = \sum\limits_{i=1}^{n-1} \frac{C(r_i+1)-C(r_i)}{C(r_n)}\log r_i }
Ein Vergleich dieser Methode mit der im vorigen Abschnitt beschriebenen zeigt, da"s der
Takens' Sch"atzer bei bekannten Systemen im allgemeinen bessere Werte liefert (s. \psref{corrdimcomp}).
\inkorrektur(Fehlerabsch"atzung)

\epsfigdouble{corrint/perfect/corrdim700b}{corrint/perfect/takdim700b}
{
...
Das Korrelationsintegral in \psref{corrintperf} ist nahezu optimal. Der Skalierungsbereich 
erstreckt sich "uber einige Gr"o"senordnungen ($\rmax/\rmin\simeq 54)$ und die Steigung innerhalb
dieses Bereichs ist nahezu konstant (s. \psref{corrslpperf}). Der Grund liegt vor allem
...
\eqnref{corrinthyper} erneut abgesch"atzt werden. Die sich hieraus ergebenden Korrekturen 
...
(s. \psref{corrnoise} rechts). 
\footnotetext{Vorrausgesetzt der Abstand wird "uber die Maximums- oder die Absolutnorm
bestimmt.} Zus"atzlich werden verschiedene Attraktorpunkte auf ein und denselben Punkt
...
das Korrelationsintegral der diskertisierten Daten $C'(r)$ folgt daraus
...
{Links das Korrelationsintegral zu einer Zeitreihe des Lorentzsystems dem wei"ses
...
\eqnl[cintdefac]{C(\eps) = \frac{2}{(N-K+1)(N-K)}\sum_{i<=j-K}\Theta(\eps-\norm{\x_i-\x_j}_\infty)}
F"ur $K=1$ geht dies wieder in die urspr"ungliche Form von \eqnref{cintdef2} "uber. Bei
...
(s. \psref{corrintlac}). Der Einflu"s der  Lakunarit"at kann minimiert werden, wenn bei
...
{Korrelationsintegrale und Steigungskurven f"ur die Cantormengen aus \psref{lacunarity}
