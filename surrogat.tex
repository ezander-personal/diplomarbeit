...
\comment{
  ...
  {Links oben Zeitreihe $1/f^2$ Rauschen. Rechts oben Fourier-Spektrum. Links unten
...
ab\footnotemark.  \footnotetext{In der Tat ist das hier gew"ahlte $T$ nicht die optimale
  ...
  eine Gau"s-Verteilung besitzen. Dies ist f"ur die folgenden Betrachtungen jedoch ohne
  Belang.}
...
\eqnref{teststatistik1} f"ur alle m"oglichen Gau"s-Prozesse $\gauss(\mu,\sigma^2)$ zu
...
\footnotetext{Ein Gau"s-Proze"s $\gauss(\mu,\sigma^2)$ erzeugt Zufallszahlen
...
G"ute verglichen werden (siehe \psref{typicalreal}). 
F"ur die Berechnung
...
durch einen Gau"s-Proze"s $\gauss(\hat\mu,\hat\sigma^2)$ erzeugt, wobei $\hat\mu$ bzw.\ 
...
$k=4$ (siehe \psref{ornsteincorrint}) berechnet. Die Korrelationsdimension
$D_2$ wurde "uber Takens' Sch"atzer mit oberer Grenze $\ln\rmax=-2,5$ abgesch"atzt.

F"ur das Lorenz-System wurde in der gleichen Weise eine Zeitreihe mit $N=8192$
